...
def mod_zeta_scores(ys, threshold = 3.5):
        """
    This approach is based in the publication by Whitaker, Darren A., and Kevin Hayes: "A simple algorithm for despiking Raman spectra." 
    ...
    threshold (float, optional): A threshold value to identify significant changes in the intensity denoting a spike. Defaults to ???.

    Returns:
    array: An array of indices where posible spikes are present (or where the second derivative of the intensity exceeds the threshold).
    """
    
    # two ways to calculate the derivative: Like this, and with np.diff. I saw before that the np.diff gave trouble when working with spectra (matrix)
    dist = 0
    ys_diff = []   
    ...
    # This creates a boolean array where True (1) indicates a point where the absolute value of the derivative exceeds the threshold (spike).

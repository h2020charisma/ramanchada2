...
def metric(y):
    l2o1 = [2*y[i-1]-y[i-2] for i in range(3, len(y)-3)]
    r2o1 = [2*y[i+1]-y[i+2] for i in range(3, len(y)-3)]
    l2o2 = [3*y[i-2]-2*y[i-3] for i in range(3, len(y)-3)]
    r2o2 = [3*y[i+2]-2*y[i+3] for i in range(3, len(y)-3)]
    y = y[3:-3]
    metric = np.max([y-np.max([l2o2, r2o2, l2o1, r2o1], axis=0), np.min([l2o2, r2o2, l2o1, r2o1], axis=0)-y], axis=0)

...
from .algos import (double_spike, first_derivative, laplacian, lr_n2o1,
                    lr_n2o1_n2o2_and, lr_n2o1_n2o2_and_extend,
...
def spikes_metric(y, /,
                  method: Literal[tuple(METHODS_WITH_METRIC.keys())],  # type: ignore [valid-type]
                  ):
    return METHODS_WITH_METRIC[method].metric(y)


@validate_call(config=dict(arbitrary_types_allowed=True))
def spikes_bool_hot(y, /,
                    method: Literal[tuple(METHODS.keys())],  # type: ignore [valid-type]
                    **kwargs):
    return METHODS[method].bool_hot(y, **kwargs)
...
def spikes_indices(y, /,
                   method: Literal[tuple(METHODS.keys())],  # type: ignore [valid-type]
                   **kwargs):
    return np.where(METHODS[method].bool_hot(y, **kwargs))[0]


@validate_call(config=dict(arbitrary_types_allowed=True))
def spikes_indices_(y, /,
                    method: Literal[tuple(METHODS.keys())],  # type: ignore [valid-type]
                    **kwargs):
    return METHODS[method].indices_(y, **kwargs)
...
def spikes_drop(x0, y0, /,
                method: Literal[tuple(METHODS.keys())],  # type: ignore [valid-type]
                **kwargs):
    idx = spikes_indices(y0, method=method, **kwargs)
    x = np.delete(x0, idx)
    y = np.delete(y0, idx)
    return x, y

...
def synthetic_spectra_array(size: PositiveInt = 100000,
    ...
    if filename:
        a = np.memmap(filename, dtype=spe_type, mode=mode, shape=size)
    else:
        a = np.zeros(size, dtype=spe_type)
    logger.info(f'{a.nbytes/1024/1024}MB used')
    return a


def generate_spiked_spectrum(inplace: Union[npt.NDArray, dict], *,
                             x: npt.NDArray,
                             ...
                             spike_fine_pos_fn: Callable[[], confloat(gt=-.5, lt=.5)
    ...
    Args:
        inplace (Union[npt.NDArray, dict]): single element from an array generated by `synthetic_spectra_array()`.

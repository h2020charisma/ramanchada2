...
def test_add_poisson_noise():
    ...
    spen1 = spe.add_poisson_noise(scale=.01, rng_seed=None)
    spen2 = spe.add_poisson_noise(scale=.01, rng_seed=None)
    assert not np.allclose(spen1.y, spen2.y)

    spen1 = spe.add_poisson_noise(scale=.01, rng_seed=123)
    spen2 = spe.add_poisson_noise(scale=.01, rng_seed=124)
    assert not np.allclose(spen1.y, spen2.y)

    spen1 = spe.add_poisson_noise(scale=.01, rng_seed=123)
    spen2 = spe.add_poisson_noise(scale=.01, rng_seed=123)
    assert np.allclose(spen1.y, spen2.y)

    rng = np.random.default_rng()
    state0 = rng.bit_generator.state
    spen1 = spe.add_poisson_noise(scale=.01, rng_seed=state0)
    ...
    spen1 = spe.add_poisson_noise(scale=.01, rng_seed=state0)
    assert state0['state']['state'] != state1['state']['state']
    spen2 = spe.add_poisson_noise(scale=.01, rng_seed=state1)
    assert state0['state']['state'] == state1['state']['state']
    assert np.allclose(spen1.y, spen2.y)
